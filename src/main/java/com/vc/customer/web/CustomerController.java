...
public class CustomerController {
	...
	public ResponseEntity<Customer> create(@RequestBody Customer c) {
		return ResponseEntity.ok(svc.createCustomer(c));
	}

	@GetMapping("/{id}")
	public ResponseEntity<Customer> get(@PathVariable Long id) {
		Customer c = svc.getCustomer(id);
		return (c == null) ? ResponseEntity.notFound().build() : ResponseEntity.ok(c);
	}
	
	@PostMapping("/purchaseOrder")
	public ResponseEntity<Customer> createPurchaseOrder(@RequestBody Customer c) {
		return ResponseEntity.ok(svc.createPurchaseOrder(c));
	}
